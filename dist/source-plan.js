...
const unsignedByte = (value, field) => {
...
const validatePlanPath = (value) => {
    if (!isNucleusSourceIdentity(value)) {
        throw new NucleusSourcePlanError(`source path ${NUCLEUS_SOURCE_IDENTITY_REQUIREMENT}`);
    }
};
export const parseNucleusSourcePlan = (source) => {
    if (/\r(?!\n)/.test(source)) {
        throw new NucleusSourcePlanError("source plan contains a lone carriage return");
    }
    ...
    if (count === 0) {
        throw new NucleusSourcePlanError("source plan must contain at least one part");
    }
    if (lines.length !== count + 2 || lines.at(-1) !== "END") {
        throw new NucleusSourcePlanError("source plan must contain its declared records followed by END");
    }
    const parts = [];
    for (let index = 0; index < count; index += 1) {
        ...
        if (Buffer.byteLength(sourcePath, "ascii") !== declaredLength) {
            throw new NucleusSourcePlanError(`source path length mismatch in record ${index + 1}`);
        }
        parts.push({ bank, path: sourcePath });
    }
    return parts;
};
export const serializeNucleusSourcePlan = (parts) => {
    if (parts.length < 1 || parts.length > 0xff) {
        throw new NucleusSourcePlanError("source-part count must be in the range 1..255");
    }
    const records = parts.map((part, index) => {
        if (!Number.isInteger(part.bank) || part.bank < 0 || part.bank > 0xff) {
            throw new NucleusSourcePlanError(`bank ordinal in record ${index + 1} must be in the range 0..255`);
        }
        const pathLength = Buffer.byteLength(part.path, "ascii");
        if (pathLength > 0xff) {
            throw new NucleusSourcePlanError("source path length must be in the range 1..255");
        }
