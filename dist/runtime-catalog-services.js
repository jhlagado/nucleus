...
const within = (memory, pointer, length) => pointer >= 0 && length >= 0 && pointer + length <= memory.length;
const contextFromMemory = (memory, pointer, services) => ({
...
export class NodeRuntimeCatalogServices {
    ...
    dispatch(memory, request) {
        ...
        const contextPointer = readWord(memory, request + NucleusRuntimeCatalogRequest.contextPointer);
        const destination = readWord(memory, request + NucleusRuntimeCatalogRequest.pointer);
        const capacity = readWord(memory, request + NucleusRuntimeCatalogRequest.capacity);
        if (memory[request + NucleusRuntimeCatalogRequest.size] !==
        ...
        try {
            const identity = readWord(memory, request + NucleusRuntimeCatalogRequest.identity);
            const expectedLength = readWord(memory, request + NucleusRuntimeCatalogRequest.expectedLength);
            const offset = readWord(memory, request + NucleusRuntimeCatalogRequest.offset);
            const image = this.provider.get(identity, contextFromMemory(memory, contextPointer, this.services));
            if (image === undefined)
                return NucleusSystemStatus.unavailable;
            if (image.identity !== identity)
                return NucleusSystemStatus.invalid;
            const selected = operation === NucleusRuntimeCatalogOperation.code
                ? image.bytes
