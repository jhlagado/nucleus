...
export const END = "⊣";
// ------------------------------------------------------------------ reading
export function readGrammar(path) {
    ...
    for (const raw of text.split("\n")) {
        ...
        if (!field)
            throw new Error(`unreadable grammar line: ${raw}`);
        const [, key, value] = field;
        if (key === "source")
            current.source = value;
        if (key === "uses")
            current.uses = value.trim() === "yes";
        if (key === "example")
...
function lex(ebnf) {
    ...
    while ((match = pattern.exec(ebnf)) !== null) {
        if (match[1] !== undefined)
            ...
            tokens.push({ kind: match[3], text: match[3] });
    }
    return tokens;
}
export function expand(productions) {
    const rules = [];
    let fresh = 0;
    const nextName = (origin, suffix) => {
        fresh += 1;
        return `${origin}·${suffix}${fresh}`;
    };
    /** Parses an alternation and returns the alternatives' symbol sequences. */
    function parseAlternation(tokens, at, origin, stop) {
        const alternatives = [];
        let current = [];
        let index = at;
        while (index < tokens.length) {
            const token = tokens[index];
            if (stop.includes(token.kind))
                break;
            if (token.kind === "|") {
...
export function firstSets(g, nullable) {
    const first = new Map();
    for (const nonterminal of g.nonterminals)
    ...
    while (changed) {
        ...
        for (const rule of g.rules) {
            ...
            for (const symbol of rule.rhs) {
                for (const t of first.get(symbol) ?? new Set())
                    target.add(t);
...
export function firstOf(sequence, first, nullable) {
...
export function followSets(g, first, nullable) {
        ...
        for (const rule of g.rules) {
            for (const [at, symbol] of rule.rhs.entries()) {
                ...
                if (restFirst.has(EPSILON)) {
                    for (const t of follow.get(rule.lhs))
                        target.add(t);
                }
                if (target.size !== before)
                    changed = true;
...
export function leftCorners(g, nullable) {
    const edges = new Map();
    for (const nonterminal of g.nonterminals)
...
export function leftRecursionCycles(edges) {
    const index = new Map();
    const low = new Map();
...
export function analyze(productions) {
    ...
    for (const rule of grammar.rules) {
        const row = table.get(rule.lhs) ?? new Map();
    ...
    for (const [nonterminal, row] of table) {
        for (const [lookahead, rules] of row) {
            ...
            for (const rule of rules) {
                for (const p of predicatesOf.get(rule.origin) ?? [])
                    declared.add(p);
            }
            collisions.push({
                nonterminal,
    ...
    return {
        ...
        follow,
        cycles,
        collisions: collisions.sort((a, b) => a.nonterminal.localeCompare(b.nonterminal) ||
...
export function shortestWitness(grammar, symbol, seen = new Set()) {
    ...
    for (const rule of grammar.rules.filter((r) => r.lhs === symbol)) {
        ...
        if (ok && (best === undefined || parts.length < best.length))
            best = parts;
    }
    return best;
}
