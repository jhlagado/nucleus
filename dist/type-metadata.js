...
export function encodeType(type) {
    ...
    if (type.kind === "u8")
        ...
        if (element.kind === "u8")
            bytes[0] = TypeTag.arrayU8;
        else if (element.kind === "u16")
            bytes[0] = TypeTag.arrayU16;
        else if (element.kind === "boolean")
            bytes[0] = TypeTag.arrayBoolean;
 ...
 */
export function metadataStorageReport(types) {
...
function requireDescriptor(descriptor) {
    if (descriptor.length !== TYPE_DESCRIPTOR_SIZE) {
        throw new Error(`type descriptor must contain ${TYPE_DESCRIPTOR_SIZE} bytes`);
    }
}
...
function positiveWord(value, label) {
