...
const requireInteger = (name, value, maximum) => {
    if (!Number.isInteger(value) || value < 0 || value > maximum) {
        fail(`${name} is outside 0..${maximum}`);
    }
};
const requireU8 = (name, value) => requireInteger(name, value, 0xff);
const requireU16 = (name, value) => requireInteger(name, value, 0xffff);
const checkedEnd = (name, base, length) => {
    requireU16(`${name} base`, base);
    requireU16(`${name} length`, length);
    const end = base + length;
    if (end > 0x10000)
        fail(`${name} wraps the Z80 address space`);
    return end;
};
const requireRegion = (name, address, length, base, capacity) => {
...
const appendU16 = (bytes, value) => {
    requireU16("u16 field", value);
    bytes.push(value & 0xff, value >>> 8);
};
const readU16 = (bytes, offset) => (bytes[offset] ?? 0) | ((bytes[offset + 1] ?? 0) << 8);
const concat = (chunks) => {
...
const record = (kind, payload) => {
    if (payload.length > 0xffff)
        fail("record payload exceeds 65,535 bytes");
...
const encodeImageLike = (kind, value) => {
...
const encodeMap = (map) => {
    ...
    for (const bank of map.partBanks)
        requireU8("MAP source-part bank", bank);
    const payload = [
        NOBJ_MAP_REVISION,
...
const validateBegin = (begin) => {
    ...
    if (begin.imageCapacity === 0)
        fail("image capacity must be nonzero");
    checkedEnd("image region", begin.imageBase, begin.imageCapacity);
    if (begin.banked) {
        if (begin.bankCount < 2)
            fail("banked BEGIN requires at least two banks");
    }
    else if (begin.bankCount !== 1) {
        fail("flat BEGIN requires exactly one bank");
    }
};
export const crc16CcittFalse = (bytes) => {
    let crc = 0xffff;
...
const crc16CcittFalseByte = (crc, byte) => {
    let next = crc ^ (byte << 8);
...
const spoolRecords = function* (spool) {
    ...
    for (;;) {
        ...
        if (low.done || high.done)
            fail("truncated spooled record header");
...
const patchHighEnds = (spool) => {
    const ends = new Map();
    for (const recordBytes of spoolRecords(spool)) {
        const interval = patchInterval(recordBytes);
        ends.set(interval.bank, Math.max(ends.get(interval.bank) ?? 0, interval.end));
    }
    return ends;
};
export class NobjGenerationSink {
    ...
    #patchHighWater = 0;
    #imageEnds = new Map();
    #patchIntervals = new Map();
    constructor(store, provider, spoolFactory = () => new MemoryNobjSpool(), options = {}) {
        this.#store = store;
    ...
    get imageSpoolHighWater() {
        return this.#imageHighWater;
    }
    get patchSpoolHighWater() {
        return this.#patchHighWater;
    }
    begin(begin) {
        if (this.#begin !== undefined)
            fail("a generation is already active");
        validateBegin(begin);
        this.#resetTentative();
        this.#begin = { ...begin };
    }
    image(bank, address, bytes) {
        ...
        this.#imageSpool.append(encodeImageLike(NobjKind.image, { bank, address, bytes }));
        this.#imageCount += 1;
        this.#imageEnds.set(bank, address + bytes.length);
        this.#imageHighWater = Math.max(this.#imageHighWater, this.#imageSpool.byteLength);
    }
    runtimeImage(bank, address, identity, context, expectedLength) {
        const begin = this.#requireOpen();
        this.#requireBeforeMap();
        ...
        if (runtime === undefined)
            fail("runtime identity is unavailable");
        ...
        if (expectedLength === 0)
            fail("runtime image must be nonempty");
        // Preflight the complete operation so a failed provider call appends no prefix.
        this.#validateImageExtent(begin, bank, address, selectedRuntime.bytes.length, "runtime IMAGE");
        this.#requireDataRecordCapacity(Math.ceil(selectedRuntime.bytes.length / NOBJ_MAX_DATA_BYTES));
        let offset = 0;
        while (offset < selectedRuntime.bytes.length) {
            const length = Math.min(NOBJ_MAX_DATA_BYTES, selectedRuntime.bytes.length - offset);
            const bytes = selectedRuntime.bytes.slice(offset, offset + length);
            this.#imageSpool.append(encodeImageLike(NobjKind.image, {
                bank,
    ...
    runtimeInitialImage(bank, address, identity, context, expectedLength) {
        const begin = this.#requireOpen();
        this.#requireBeforeMap();
        requireU16("runtime identity", identity);
        ...
        if (expectedLength === 0)
            fail("runtime initial image must be nonempty");
        this.#validateImageExtent(begin, bank, address, selectedRuntime.initialBytes.length, "runtime initial IMAGE");
        this.#requireDataRecordCapacity(Math.ceil(selectedRuntime.initialBytes.length / NOBJ_MAX_DATA_BYTES));
        ...
        while (offset < initialBytes.length) {
            const length = Math.min(NOBJ_MAX_DATA_BYTES, initialBytes.length - offset);
    ...
    patch(bank, address, bytes) {
        const begin = this.#requireOpen();
        this.#requireBeforeMap();
        this.#requireDataRecordCapacity(1);
        this.#validatePatch(begin, bank, address, bytes);
        this.#patchSpool.append(encodeImageLike(NobjKind.patch, { bank, address, bytes }));
        this.#patchCount += 1;
        ...
        this.#patchHighWater = Math.max(this.#patchHighWater, this.#patchSpool.byteLength);
    }
    map(map) {
        this.#requireOpen();
        if (this.#map !== undefined)
            fail("MAP was already submitted");
    ...
    #commitTo(output) {
        ...
        const map = this.#map;
        if (map === undefined)
            fail("MAP is required before COMMIT");
        const committedMap = map;
        if (this.#imageCount === 0)
            fail("at least one IMAGE record is required");
        ...
        validateMapFromEnds(begin, committedMap, this.#imageEnds, patchHighEnds(this.#patchSpool));
        const commitPrefix = [NobjKind.commit, 7, 0];
        ...
        for (const chunk of this.#imageSpool.chunks())
            writeCovered(chunk);
        for (const chunk of this.#patchSpool.chunks())
            writeCovered(chunk);
    ...
    #requireOpen() {
        if (this.#begin === undefined)
            fail("no NOBJ generation is active");
        return this.#begin;
    }
    #requireBeforeMap() {
        if (this.#map !== undefined)
    ...
    #validateImageExtent(begin, bank, address, length, name) {
        ...
        const end = requireRegion(name, address, length, begin.imageBase, begin.imageCapacity);
        ...
        if (end > 0x10000)
            fail(`${name} crosses the Z80 address space`);
    }
    #validatePatch(begin, bank, address, bytes) {
        if (bytes.length < 1 || bytes.length > NOBJ_MAX_DATA_BYTES) {
            fail("PATCH byte count is outside 1..65,532");
        }
        requireU8("PATCH bank", bank);
        if (bank >= begin.bankCount)
            fail("PATCH bank is outside BEGIN.bankCount");
        const end = requireRegion("PATCH", address, bytes.length, begin.imageBase, begin.imageCapacity);
...
const decodeBegin = (bytes, recordValue) => {
...
const decodeImageLike = (bytes, recordValue, name) => {
    const length = recordValue.payloadEnd - recordValue.payloadStart;
    if (length < 4)
        fail(`${name} payload length must be at least 4`);
    const p = recordValue.payloadStart;
    return {
        bank: bytes[p] ?? 0,
...
const decodeMap = (bytes, recordValue) => {
    const p = recordValue.payloadStart;
    const length = recordValue.payloadEnd - p;
    if (length < 41)
        fail("MAP payload is shorter than its fixed fields and one bank");
    if (bytes[p] !== NOBJ_MAP_REVISION)
        fail("unsupported MAP revision");
    const flags = bytes[p + 1] ?? 0;
    if ((flags & 0xfc) !== 0)
        fail("MAP contains reserved flags");
    const partCount = bytes[p + 28] ?? 0;
    if (partCount === 0)
        fail("MAP part count must be nonzero");
    ...
    if (length !== expectedLength)
        fail("MAP payload length is inconsistent");
    const banks = [];
...
const validateMap = (begin, map, images, patches) => {
    requireU8("MAP entry bank", map.entryBank);
    ...
    if (map.writableCapacity === 0)
        fail("MAP writable capacity must be nonzero");
    const writableEnd = checkedEnd("MAP writable region", map.writableBase, map.writableCapacity);
    ...
    if (map.entryBank >= begin.bankCount)
        fail("MAP entry bank is out of range");
    ...
    for (const bank of map.partBanks) {
        if (bank >= begin.bankCount)
            fail("MAP source-part bank is out of range");
    }
    ...
    const regionsOverlap = begin.imageBase < writableEnd && map.writableBase < imageEnd;
    const writableInsideImage = map.writableBase >= begin.imageBase && writableEnd <= imageEnd;
    if (map.romMode) {
        if (regionsOverlap)
            fail("ROM-mode writable and image regions overlap");
    }
    else if (!writableInsideImage) {
        fail("loaded-mode writable region is not wholly inside the image region");
    }
    if (map.vectorBase !== map.writableBase ||
    ...
    const initializedEnd = checkedEnd("MAP initialized run", map.initializedRunBase, map.initializedRunLength);
    ...
    const highestEnds = Array.from({ length: begin.bankCount }, () => begin.imageBase);
    for (const item of [...images, ...patches]) {
        if (item.bank >= begin.bankCount)
            fail("record bank is outside BEGIN.bankCount");
        const end = requireRegion("object record", item.address, item.bytes.length, begin.imageBase, begin.imageCapacity);
        highestEnds[item.bank] = Math.max(highestEnds[item.bank] ?? begin.imageBase, end);
    }
    for (let bankIndex = 0; bankIndex < map.banks.length; bankIndex += 1) {
        const bank = map.banks[bankIndex];
        if (bank === undefined)
            fail("MAP bank entry is missing");
        ...
        for (const item of [...images, ...patches]) {
            if (item.bank === bankIndex &&
                item.address + item.bytes.length > usedEnd) {
                fail("record lies beyond MAP.usedLength");
            }
        }
        validateOptionalImageExtent("read-only", bank.readOnlyBase, bank.readOnlyLength, begin.imageBase, usedEnd);
        validateOptionalImageExtent("aggregate-constant", bank.aggregateConstantBase, bank.aggregateConstantLength, begin.imageBase, usedEnd);
        if (bank.aggregateConstantLength > 0) {
            const readOnlyEnd = bank.readOnlyBase + bank.readOnlyLength;
            const aggregateEnd = bank.aggregateConstantBase + bank.aggregateConstantLength;
            if (bank.readOnlyLength === 0 ||
    ...
    const entryUsedEnd = begin.imageBase + (map.banks[map.entryBank]?.usedLength ?? 0);
    ...
    if (map.romMode) {
        const loadUsedEnd = begin.imageBase + (map.banks[map.dataLoadBank]?.usedLength ?? 0);
        const loadEnd = checkedEnd("MAP data-load extent", map.dataLoadAddress, map.dataLoadLength);
        if (map.dataLoadAddress < begin.imageBase || loadEnd > loadUsedEnd) {
            fail("MAP data-load extent is outside its bank used extent");
        }
    }
};
const validateMapFromEnds = (begin, map, imageEnds, patchEnds) => {
    const synthetic = (ends) => [...ends.entries()]
...
const validateOptionalImageExtent = (name, base, length, imageBase, usedEnd) => {
    requireU16(`${name} base`, base);
    requireU16(`${name} length`, length);
    if (length === 0) {
        if (base !== 0)
            fail(`zero-length ${name} extent must have base zero`);
        return;
    }
...
export class NobjStreamReader {
    ...
    push(chunk) {
        ...
        const available = this.#pending.length === 0 ? chunk : concat([this.#pending, chunk]);
        let cursor = 0;
    ...
    #acceptRecord(bytes) {
        ...
        if (!Object.values(NobjKind).includes(kind)) {
    ...
    #acceptImage(bytes, recordValue) {
        if (this.#phase !== "image")
            fail("IMAGE appears outside the IMAGE phase");
        if (this.#begin === undefined)
            fail("IMAGE appears before BEGIN");
        const begin = this.#begin;
        const item = decodeImageLike(bytes, recordValue, "IMAGE");
        if (item.bank >= begin.bankCount)
            fail("IMAGE bank is out of range");
        const end = requireRegion("IMAGE", item.address, item.bytes.length, begin.imageBase, begin.imageCapacity);
    ...
    #acceptPatch(bytes, recordValue) {
        if (this.#phase !== "image" && this.#phase !== "patch")
            fail("PATCH appears outside the PATCH phase");
        if (this.#imageCount === 0)
            fail("PATCH requires at least one IMAGE");
        if (this.#begin === undefined)
            fail("PATCH appears before BEGIN");
        const begin = this.#begin;
        this.#phase = "patch";
        const item = decodeImageLike(bytes, recordValue, "PATCH");
        if (item.bank >= begin.bankCount)
            fail("PATCH bank is out of range");
        const end = requireRegion("PATCH", item.address, item.bytes.length, begin.imageBase, begin.imageCapacity);
        ...
        this.#patchEnds.set(item.bank, Math.max(this.#patchEnds.get(item.bank) ?? 0, end));
        this.#options.onPatch?.(item);
    }
    #acceptCommit(bytes, recordValue) {
...
export const validateRewindableNobjChunks = (chunks) => {
    let patchCount = 0;
    ...
    for (let outerIndex = 0; outerIndex < patchCount; outerIndex += 1) {
        ...
        if (outer === undefined)
            fail("PATCH rescan count changed");
        const activeOuter = outer;
        scan((record, index) => {
            if (index >= outerIndex || record.bank !== activeOuter.bank)
                return;
...
export const materializeNobjChunks = (chunks) => {
    let begin;
    let banks = [];
    const apply = (record) => {
        if (begin === undefined)
            fail("object data appears before BEGIN");
        const activeBegin = begin;
        const bank = banks[record.bank];
        if (bank === undefined)
            fail("materializer bank is unavailable");
...
export const parseNobj = (serialized) => {
    const records = [];
    let cursor = 0;
        ...
        switch (recordValue.kind) {
            ...
            case NobjKind.image: {
                ...
                if (item.bank >= activeBegin.bankCount)
                    fail("IMAGE bank is out of range");
                const end = requireRegion("IMAGE", item.address, item.bytes.length, activeBegin.imageBase, activeBegin.imageCapacity);
            ...
            case NobjKind.patch: {
                ...
                if (item.bank >= activeBegin.bankCount)
                    fail("PATCH bank is out of range");
                const end = requireRegion("PATCH", item.address, item.bytes.length, activeBegin.imageBase, activeBegin.imageCapacity);
            ...
            case NobjKind.commit: {
                ...
                if (recordCount !== records.length)
                    fail("COMMIT record count is incorrect");
                if (entryBank !== activeMap.entryBank ||
    ...
    const completedBegin = begin;
    const completedMap = map;
    const completedCommit = commit;
...
export const materializeNobj = (parsed) => {
