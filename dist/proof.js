...
import { materializeNobj, MemoryNobjSpool, NobjGenerationSink, NobjGenerationStore, } from "./nobj.js";
...
export async function runProofManifest(manifestFile) {
    ...
    const memoryProfile = readJson(path.resolve(manifestDirectory, manifest.memoryProfile));
    ...
    const assembled = await compile(sourcePath, {
        ...
        registerContractsInterfaces: (manifest.interfaces ?? []).map((file) => path.resolve(manifestDirectory, file)),
    });
    const errors = assembled.diagnostics.filter((diagnostic) => diagnostic.severity === "error");
    if (errors.length > 0) {
        throw new ProofFailure(`${manifest.name}: assembly failed\n${errors
            .map((diagnostic) => `  ${diagnostic.sourceName ?? sourcePath}:${diagnostic.line ?? "?"}:${diagnostic.column ?? "?"} ${diagnostic.message}`)
            .join("\n")}`);
    }
    ...
    const symbols = Object.fromEntries(debugMap.json.symbols.flatMap((entry) => {
    ...
    for (const [name, expected] of Object.entries(manifest.symbols ?? {})) {
        ...
        if (actual !== expected) {
            throw new ProofFailure(`${manifest.name}: symbol ${name} is ${hexWord(actual)}, expected ${hexWord(expected)}`);
        }
    }
    const regions = memoryProfile.regions.map((region) => {
        ...
        if (start < 0 || end > 0x10000 || bytes <= 0) {
            throw new ProofFailure(`${manifest.name}: invalid ${region.name} region ${hexWord(start)}..${hexWord(end)}`);
        }
        if (bytes > region.maxBytes) {
            throw new ProofFailure(`${manifest.name}: ${region.name} uses ${bytes} bytes, limit ${region.maxBytes}`);
        }
    ...
    if (orderedRegions[0]?.start !== 0) {
        throw new ProofFailure(`${manifest.name}: memory profile does not start at $0000`);
    }
    for (let index = 1; index < orderedRegions.length; index += 1) {
        ...
        if (previous && current && previous.end !== current.start) {
            throw new ProofFailure(`${manifest.name}: ${previous.name} and ${current.name} do not meet`);
        }
    }
    if (memoryProfile.addressSpaceBytes !== 0x10000 ||
        orderedRegions.at(-1)?.end !== memoryProfile.addressSpaceBytes) {
        throw new ProofFailure(`${manifest.name}: memory profile does not cover exactly 64 KiB`);
    }
    const extents = (manifest.extents ?? []).map((extent) => {
        ...
        if (bytes < 0 || bytes > extent.maxBytes) {
            throw new ProofFailure(`${manifest.name}: ${extent.name} uses ${bytes} bytes, limit ${extent.maxBytes}`);
        }
    ...
    for (const write of manifest.writes ?? []) {
        ...
        if (bytes === undefined) {
            throw new ProofFailure(`${manifest.name}: write at ${write.at} has no bytes`);
        }
        const address = symbolValue(write.at);
        if (address < 0 || address + bytes.length > memory.length) {
            throw new ProofFailure(`${manifest.name}: write at ${write.at} is out of range`);
        }
        memory.set(bytes, address);
    }
    let cycles = 0;
    let instructions = 0;
    const recentProgramCounters = [];
    ...
    if (runtime.isHalted() !== manifest.execution.halted) {
        failures.push(`halted=${runtime.isHalted()}, expected ${manifest.execution.halted}`);
    }
    if (instructions > manifest.execution.maxInstructions) {
        failures.push(`instruction limit ${manifest.execution.maxInstructions} exceeded`);
    }
    ...
    for (const observation of manifest.observations ?? []) {
        ...
        const actual = observation.width === "u8"
            ? memory[address]
            : (memory[address] ?? 0) | ((memory[address + 1] ?? 0) << 8);
        if (actual !== observation.equals) {
            failures.push(`${observation.at}=${actual}, expected ${observation.equals}`);
        }
    }
...
const runNobjManifest = async (name, manifest, producerMemory, symbol) => {
    ...
    const runtimeLinkContext = manifest.runtimeLinkContext ?? defaultRuntimeLinkContext;
...
const prepareNobjAdapterGeneration = async ({ name, producerMemory, start, length, maxBytes, begin, map, runtimeLinkContext = defaultRuntimeLinkContext, store = new NobjGenerationStore(), spoolFactory, lowMemoryPatchValidation = false, onImageByte, }) => {
    if (length > maxBytes) {
        throw new ProofFailure(`${name}: NOBJ adapter log uses ${length} bytes, limit ${maxBytes}`);
    }
    ...
    const sink = new NobjGenerationSink(store, provider, spoolFactory ?? (() => new MemoryNobjSpool()), { lowMemoryPatchValidation });
    sink.begin(begin);
    try {
        let cursor = start;
...
export const commitNobjAdapterGeneration = async (generation) => (await prepareNobjAdapterGeneration(generation)).commit();
export const commitNobjAdapterGenerationTo = async (generation, output) => (await prepareNobjAdapterGeneration(generation)).commitTo(output);
export const executeCommittedNobj = (serialized, execution, options = {}) => {
    const parsed = parseNobjForExecution(serialized);
    const materialized = materializeNobj(parsed);
    ...
    if (parsed.begin.banked) {
        if (switchConfig === undefined) {
            throw new ProofFailure("banked NOBJ execution requires a bank-switch hook");
        }
    }
    let instructions = 0;
    let cycles = 0;
    ...
    if (execution.expectedSp !== undefined &&
        runtime.cpu.sp !== execution.expectedSp) {
        failures.push(`SP=${hexWord(runtime.cpu.sp)}, expected ${hexWord(execution.expectedSp)}`);
    }
    if (execution.expectedIx !== undefined &&
    ...
    for (const observation of options.observations ?? []) {
        ...
        const offset = observation.bank === undefined
            ? observation.at
        ...
        if (actual !== observation.equals) {
            failures.push(`NOBJ observation at ${hexWord(observation.at)}=${actual}, expected ${observation.equals}`);
        }
    }
    if (failures.length > 0) {
...
const parseNobjForExecution = (serialized) => {
