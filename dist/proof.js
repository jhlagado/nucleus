...
export const commitNobjAdapterGeneration = async ({ name, producerMemory, start, length, maxBytes, begin, map, runtimeLinkContext = defaultRuntimeLinkContext, store = new NobjGenerationStore(), onImageByte, }) => {
    ...
    while (cursor < end) {
        ...
        if (kind === 3 || kind === 4) {
            ...
            cursor += 2;
            const contextWord = (offset) => (producerMemory[cursor + offset] ?? 0) |
            ...
            for (const field of [
                ...
                if (compilerContext[field] !== runtimeLinkContext[field]) {
                    throw new ProofFailure(`${name}: runtime link context ${field} differs from the compiler operation`);
                }
            }
            if (kind === 3) {
                ...
                sink.runtimeInitialImage(bank, address, identity, compilerContext, count);
            }
            continue;
        }
        if (kind !== 1 && kind !== 2) {
