...
import { parseNucleusSourcePlan, serializeNucleusSourcePlan, } from "./source-plan.js";
import { mapNucleusSourceBundleOffset, prepareNucleusSourceBundle, } from "./source-bundle.js";
...
const nativeCompilerImages = new Map();
...
const validateNativeHostVector = (image) => {
    ...
    const expectedHeader = [0x4e, 0x48, 0, 1, 8, 14, 0, 0];
...
const prepareNativeSource = (parts, requestedBanks) => {
    ...
    const bundle = prepareNucleusSourceBundle(parts.map((part, index) => ({
        name: part.name,
        source: bytes[index],
...
const createNativeSourceObjects = (parts, prepared) => {
    ...
    try {
        ...
        const banksByName = new Map(parts.map((part, index) => [part.name, prepared.partBanks[index]]));
        writeFileSync(planPath, serializeNucleusSourcePlan(plan.map(({ path: sourcePath }) => ({
            path: sourcePath,
...
const capturedMap = (memory, symbols, begin, target, partBanks) => {
    const address = (name) => symbol(symbols, name);
    const value = (name) => readWord(memory, address(name));
        ...
        banks: [
            {
                ...
                readOnlyLength,
                aggregateConstantBase: aggregateLength === 0 ? 0 : value("TargetContextRoDataBase"),
                aggregateConstantLength: aggregateLength,
            },
        ],
    };
};
const capturedContext = (map, begin, staticLength, services) => ({
...
const nativeMapRequest = (memory, request) => {
    const byte = (offset) => memory[request + offset] ?? 0;
    const word = (offset) => readWord(memory, request + offset);
    if (byte(0) !== 1)
        throw new Error("native MAP request revision is invalid");
    const flags = byte(1);
    if ((flags & ~3) !== 0)
        throw new Error("native MAP flags are invalid");
    ...
    const partBanks = Array.from(memory.slice(partBanksPointer, partBanksPointer + partCount));
    const banks = Array.from({ length: bankCount }, (_, bank) => {
        ...
        const entryInitializedLength = isEntry && romMode ? initializedRunLength : 0;
        ...
        return {
            ...
            readOnlyLength,
            aggregateConstantBase: aggregateConstantLength === 0 ? 0 : aggregateConstantBase,
...
const nativeRuntimeContext = (memory, pointer, services) => ({
...
const validateNativeTargetDescriptor = (memory, pointer, begin, target, partBanks) => {
...
const runNucleusCompilerNativeTo = async (parts, target, output, options) => {
    ...
    const runtimeCatalogServices = new NodeRuntimeCatalogServices(provider, target.services ?? defaultNucleusServices);
            ...
            try {
                if (objectServiceCall) {
                    ...
                    if (status !== NucleusSystemStatus.success) {
                        nativeHostFailure = new Error(`native named object failed with status ${status} (size=${memory[hl]}, abi=${memory[hl + 1]}, operation=${memory[hl + 2]}, flags=${memory[hl + 3]}, handle=${readWord(memory, hl + 4)}, pointer=${readWord(memory, hl + 6)}, length=${readWord(memory, hl + 8)}, offset=${readWord(memory, hl + 10)}:${readWord(memory, hl + 12)}, result=${readWord(memory, hl + 14)})`);
                    }
                    cpu.a = status;
                    ...
                    if (status !== NucleusSystemStatus.success) {
                        nativeHostFailure = new Error(`native runtime catalogue failed with status ${status} (operation=${memory[hl + 2]}, offset=${readWord(memory, hl + 12)}, capacity=${readWord(memory, hl + 16)})`);
                    }
                    cpu.a = status;
                    cpu.flags.C = status === NucleusSystemStatus.success ? 0 : 1;
                    return;
                    ...
                    if (value === 0 &&
                        ...
                        const serialized = Uint8Array.from(readFileSync(path.join(nativeSourceObjects.root, ".nucleus/program.nobj")));
                    ...
                    const placement = prepared.bundle.placement.find((range) => sourceOffset >= range.start && sourceOffset < range.end);
                    ...
                    const chunkLength = Math.min(chunkLimit - chunkBase, placement.end - sourceOffset);
                    if (chunkLength <= 0) {
                        throw new Error("native source provider produced an empty chunk");
                    }
                    memory.set(prepared.bundle.bytes.subarray(sourceOffset, sourceOffset + chunkLength), chunkBase);
                    ...
                    if (length === 0 ||
                        ...
                        throw new Error(`native retained-name request is invalid (bank=${cpu.c}, offset=${de}, length=${length}, pointer=${hl.toString(16)}, source=${source.length})`);
                    }
                    for (let index = 0; index < length; index += 1) {
                        if (memory[hl + index] !== source[de + index]) {
                            throw new Error(`native retained name differs from source (bank=${cpu.c}, offset=${de}, length=${length}, pointer=${hl.toString(16)}, index=${index}, actual=${memory[hl + index]}, expected=${source[de + index]})`);
                        }
                    }
                    ...
                    sink = new NobjGenerationSink(new NobjGenerationStore(), provider, options.spoolFactory ?? (() => new MemoryNobjSpool()), { lowMemoryPatchValidation: options.lowMemoryPatchValidation });
                    sink.begin(begin);
                    succeed();
                }
                else if (selectedPort ===
                ...
                else if (selectedPort === symbol(image.symbols, "NativeHostMapFlatPort") ||
    ...
    if (debugHooks) {
        collector = new NucleusDebugCollector(memory, prepared.loaded, debugTraceSymbols(image), (handle, length) => {
            ...
            const mapped = mapNucleusSourceBundleOffset(prepared.bundle, retained.offset);
            const part = prepared.loaded.find((candidate) => candidate.name === mapped.name);
...
export const compileNucleusTo = async (parts, target, output, options = {}) => runNucleusCompilerNativeTo(parts, target, output, options);
