...
describe("manifest-driven AZM and Debug80 proofs", () => {
  it("publishes a flat target through the append-only logical sink", async () => {
    ...
    expect(outcome.instructions).toBe(884_952);
    expect(outcome.cycles).toBe(9_491_133);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 15_566 },
      { name: "compiler-immutable", bytes: 437 },
      { name: "compiler-core", bytes: 16_003 },
  ...
  it("fits and executes the complete host-instrumented compiler layout", async () => {
    ...
    expect(outcome.memory[outcome.symbols.ProofStatus ?? -1]).toBe(0xa5);
    expect(outcome.memory[outcome.symbols.ProofCase ?? -1]).toBe(0);
    expect(outcome.instructions).toBe(888_577);
    expect(outcome.cycles).toBe(9_531_098);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 15_630 },
      { name: "compiler-immutable", bytes: 437 },
      { name: "compiler-core", bytes: 16_067 },
      { name: "instrumented-core-gap", bytes: 8_509 },
      { name: "compiler-workspace", bytes: 3_613 },
      { name: "selected-proof-runtime", bytes: 899 },
  ...
  it("retains the historical direct-Z80 Chapter 21 module proof", async () => {
    ...
    expect(outcome.instructions).toBe(1_482_797);
    expect(outcome.cycles).toBe(14_601_493);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 15_137 },
      { name: "compiler-immutable", bytes: 437 },
      { name: "compiler-core", bytes: 15_574 },
  ...
  it("retains the historical direct-Z80 Stage 8 module proof", async () => {
    ...
    expect(outcome.instructions).toBe(1_758_196);
    expect(outcome.cycles).toBe(16_996_841);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 15_137 },
      { name: "compiler-immutable", bytes: 437 },
      { name: "compiler-core", bytes: 15_574 },
  ...
  it("checks the scalar-local and counted-loop source slice", async () => {
    ...
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 8_481 },
      { name: "compiler-immutable", bytes: 279 },
      { name: "compiler-core", bytes: 8_760 },
  ...
  it("executes the counted loop as direct Z80", async () => {
    ...
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_481 },
      { name: "z80-output-sink", bytes: 1_511 },
      { name: "compiler-code", bytes: 9_992 },
      { name: "compiler-immutable", bytes: 279 },
      { name: "compiler-core", bytes: 10_271 },
      { name: "compiler-workspace", bytes: 1_566 },
  ...
  it("executes checked initialized-array selection as direct Z80", async () => {
    ...
    expect(outcome.cycles).toBe(539_648);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_481 },
  ...
  it("executes general scalar symbols and precedence as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(108_004);
    expect(outcome.cycles).toBe(1_147_324);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_481 },
      { name: "source-adapter", bytes: 86 },
      { name: "tokenizer", bytes: 648 },
      ...
      { name: "parser", bytes: 7_549 },
      { name: "z80-output-sink", bytes: 3_396 },
      { name: "z80-expression-backend", bytes: 351 },
      { name: "compiler-code", bytes: 11_877 },
      { name: "compiler-immutable", bytes: 279 },
      { name: "compiler-core", bytes: 12_156 },
      { name: "compiler-workspace", bytes: 1_566 },
  ...
  it("executes typed scalar expressions and traps atomically as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(619_079);
    expect(outcome.cycles).toBe(6_395_755);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_481 },
      { name: "source-adapter", bytes: 86 },
      { name: "tokenizer", bytes: 648 },
      ...
      { name: "compiler-code", bytes: 10_777 },
      { name: "compiler-immutable", bytes: 279 },
      { name: "compiler-core", bytes: 11_056 },
      { name: "compiler-workspace", bytes: 1_566 },
  ...
  it("executes typed structured control as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(250_018);
    expect(outcome.cycles).toBe(2_660_410);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_481 },
      { name: "parser", bytes: 7_549 },
  ...
  it("emits packed aggregate layouts and publishes one atomic static image", async () => {
    ...
    expect(outcome.memory[outcome.symbols.ProofStatus ?? -1]).toBe(0xa5);
    expect(outcome.memory[outcome.symbols.ProofCase ?? -1]).toBe(0);
    expect(outcome.instructions).toBe(298_117);
    expect(outcome.cycles).toBe(3_028_559);
    expect(
      outcome.extents.find(({ name }) => name === "compiler-core")?.bytes,
    ).toBeLessThanOrEqual(16_384);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 7_162 },
      { name: "parser", bytes: 6_233 },
      { name: "typed-z80-sink", bytes: 1_931 },
      { name: "compiler-code", bytes: 9_504 },
      { name: "compiler-immutable", bytes: 263 },
      { name: "compiler-core", bytes: 9_767 },
      { name: "compiler-workspace", bytes: 1_566 },
