...
describe("manifest-driven AZM and Debug80 proofs", () => {
  it("publishes a flat target through the append-only logical sink", async () => {
    ...
    expect(outcome.instructions).toBe(1_052_768);
    expect(outcome.cycles).toBe(10_364_607);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 15_477 },
      { name: "compiler-immutable", bytes: 401 },
      { name: "compiler-core", bytes: 15_878 },
      { name: "compiler-workspace", bytes: 3_611 },
      { name: "selected-proof-runtime", bytes: 600 },
      { name: "proof-code-and-data", bytes: 2_487 },
    ]);
    expect(outcome.nobj?.parsed.begin.runtimeIdentity).toBe(5);
    expect(outcome.nobj?.parsed.map.entryAddress).toBe(0x8000);
    expect(outcome.nobj?.parsed.map.banks[0]?.usedLength).toBe(582);
    ...
    expect(
      Array.from(outcome.nobj?.memory.slice(0x820a, 0x820c) ?? []),
    ).toEqual([3, 0]);
    const generated = Array.from(
      outcome.nobj?.memory.slice(0x820c, 0x8246) ?? [],
    );
  ...
  it("fits and executes the complete host-instrumented compiler layout", async () => {
    ...
    expect(outcome.memory[outcome.symbols.ProofStatus ?? -1]).toBe(0xa5);
    expect(outcome.memory[outcome.symbols.ProofCase ?? -1]).toBe(0);
    expect(outcome.instructions).toBe(1_057_345);
    expect(outcome.cycles).toBe(10_415_046);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 15_533 },
      { name: "compiler-immutable", bytes: 401 },
      { name: "compiler-core", bytes: 15_934 },
      { name: "instrumented-core-gap", bytes: 1_474 },
      { name: "compiler-workspace", bytes: 3_611 },
      { name: "selected-proof-runtime", bytes: 600 },
  ...
  it("runs the accepted Chapter 21 multipart program through committed NOBJ", async () => {
    ...
    expect(outcome.nobj?.parsed.map.banks[0]?.usedLength).toBe(1487);
  ...
  it("retains the historical direct-Z80 Chapter 21 module proof", async () => {
    ...
    expect(outcome.instructions).toBe(1_658_551);
    expect(outcome.cycles).toBe(15_625_487);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 14_594 },
      { name: "compiler-immutable", bytes: 401 },
      { name: "compiler-core", bytes: 14_995 },
      { name: "compiler-workspace", bytes: 3_605 },
      { name: "generated-z80-bound", bytes: 4_096 },
      { name: "z80-runtime", bytes: 622 },
  ...
  it("retains the historical direct-Z80 Stage 8 module proof", async () => {
    ...
    expect(outcome.instructions).toBe(2_037_291);
    expect(outcome.cycles).toBe(18_978_680);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 14_594 },
      { name: "compiler-immutable", bytes: 401 },
      { name: "compiler-core", bytes: 14_995 },
  ...
  it("checks the scalar-local and counted-loop source slice", async () => {
    ...
    expect(outcome.instructions).toBe(50_873);
    expect(outcome.cycles).toBe(492_617);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 8_399 },
      { name: "compiler-immutable", bytes: 270 },
      { name: "compiler-core", bytes: 8_669 },
  ...
  it("executes the counted loop as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(45_541);
    expect(outcome.cycles).toBe(442_783);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_399 },
      { name: "z80-output-sink", bytes: 1_499 },
      { name: "compiler-code", bytes: 9_898 },
      { name: "compiler-immutable", bytes: 270 },
      { name: "compiler-core", bytes: 10_168 },
      { name: "compiler-workspace", bytes: 1_550 },
  ...
  it("executes checked initialized-array selection as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(65_058);
    expect(outcome.cycles).toBe(629_276);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_399 },
      { name: "source-adapter", bytes: 99 },
      { name: "tokenizer", bytes: 820 },
      { name: "semantic-sink", bytes: 66 },
      { name: "parser", bytes: 7_294 },
      { name: "z80-output-sink", bytes: 1_499 },
      { name: "compiler-code", bytes: 9_898 },
      { name: "compiler-immutable", bytes: 270 },
      { name: "compiler-core", bytes: 10_168 },
  ...
  it("executes a forward-declared recursive scalar value call", async () => {
    ...
    expect(outcome.instructions).toBe(79_023);
    expect(outcome.cycles).toBe(756_733);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_399 },
      { name: "source-adapter", bytes: 99 },
      { name: "tokenizer", bytes: 820 },
  ...
  it("executes general scalar symbols and precedence as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(136_259);
    expect(outcome.cycles).toBe(1_334_463);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_399 },
      { name: "source-adapter", bytes: 99 },
      { name: "tokenizer", bytes: 820 },
      ...
      { name: "parser", bytes: 7_294 },
      { name: "z80-output-sink", bytes: 3_314 },
      { name: "z80-expression-backend", bytes: 351 },
      { name: "compiler-code", bytes: 11_713 },
      { name: "compiler-immutable", bytes: 270 },
      { name: "compiler-core", bytes: 11_983 },
      { name: "compiler-workspace", bytes: 1_550 },
  ...
  it("executes typed scalar expressions and traps atomically as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(813_216);
    expect(outcome.cycles).toBe(7_719_397);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_399 },
      { name: "source-adapter", bytes: 99 },
      { name: "tokenizer", bytes: 820 },
      ...
      { name: "symbol-table", bytes: 120 },
      { name: "parser", bytes: 7_294 },
      { name: "typed-z80-sink", bytes: 1_815 },
      { name: "compiler-code", bytes: 10_613 },
      { name: "compiler-immutable", bytes: 270 },
      { name: "compiler-core", bytes: 10_883 },
      { name: "compiler-workspace", bytes: 1_550 },
  ...
  it("executes typed structured control as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(316_772);
    expect(outcome.cycles).toBe(3_089_004);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_399 },
      { name: "parser", bytes: 7_294 },
      { name: "structured-z80-sink", bytes: 1_815 },
      { name: "compiler-code", bytes: 10_613 },
      { name: "compiler-immutable", bytes: 270 },
      { name: "compiler-core", bytes: 10_883 },
      { name: "compiler-workspace", bytes: 1_550 },
  ...
  it("emits packed aggregate layouts and publishes one atomic static image", async () => {
    ...
    expect(outcome.memory[outcome.symbols.ProofStatus ?? -1]).toBe(0xa5);
    expect(outcome.memory[outcome.symbols.ProofCase ?? -1]).toBe(0);
    expect(outcome.instructions).toBe(377_087);
    expect(outcome.cycles).toBe(3_543_378);
    expect(
      outcome.extents.find(({ name }) => name === "compiler-core")?.bytes,
    ).toBeLessThanOrEqual(16_384);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 7_080 },
      { name: "parser", bytes: 5_978 },
      { name: "typed-z80-sink", bytes: 1_861 },
      { name: "compiler-code", bytes: 9_340 },
      { name: "compiler-immutable", bytes: 254 },
      { name: "compiler-core", bytes: 9_594 },
      { name: "compiler-workspace", bytes: 1_550 },
