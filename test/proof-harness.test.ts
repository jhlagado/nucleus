...
describe("manifest-driven AZM and Debug80 proofs", () => {
  it("publishes a flat target through the append-only logical sink", async () => {
    ...
    expect(outcome.instructions).toBe(1_055_183);
    expect(outcome.cycles).toBe(10_384_694);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 14_893 },
      { name: "compiler-immutable", bytes: 393 },
      { name: "compiler-core", bytes: 15_286 },
  ...
  it("fits and executes the complete host-instrumented compiler layout", async () => {
    ...
    expect(outcome.memory[outcome.symbols.ProofStatus ?? -1]).toBe(0xa5);
    expect(outcome.memory[outcome.symbols.ProofCase ?? -1]).toBe(0);
    expect(outcome.instructions).toBe(1_059_812);
    expect(outcome.cycles).toBe(10_435_705);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 14_949 },
      { name: "compiler-immutable", bytes: 393 },
      { name: "compiler-core", bytes: 15_342 },
      { name: "instrumented-core-gap", bytes: 2_066 },
      { name: "compiler-workspace", bytes: 3_609 },
      { name: "selected-proof-runtime", bytes: 574 },
  ...
  it("retains the historical direct-Z80 Chapter 21 module proof", async () => {
    ...
    expect(outcome.instructions).toBe(1_654_777);
    expect(outcome.cycles).toBe(15_573_136);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 13_966 },
      { name: "compiler-immutable", bytes: 393 },
      { name: "compiler-core", bytes: 14_359 },
  ...
  it("retains the historical direct-Z80 Stage 8 module proof", async () => {
    ...
    expect(outcome.instructions).toBe(2_033_464);
    expect(outcome.cycles).toBe(18_924_415);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 13_966 },
      { name: "compiler-immutable", bytes: 393 },
      { name: "compiler-core", bytes: 14_359 },
  ...
  it("checks the scalar-local and counted-loop source slice", async () => {
    ...
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 8_395 },
      { name: "compiler-immutable", bytes: 270 },
      { name: "compiler-core", bytes: 8_665 },
  ...
  it("executes the counted loop as direct Z80", async () => {
    ...
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_395 },
      { name: "z80-output-sink", bytes: 1_499 },
      { name: "compiler-code", bytes: 9_894 },
      { name: "compiler-immutable", bytes: 270 },
      { name: "compiler-core", bytes: 10_164 },
      { name: "compiler-workspace", bytes: 1_550 },
  ...
  it("executes general scalar symbols and precedence as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(135_923);
    expect(outcome.cycles).toBe(1_328_542);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_395 },
      { name: "source-adapter", bytes: 99 },
      { name: "tokenizer", bytes: 820 },
      ...
      { name: "compiler-code", bytes: 11_709 },
      { name: "compiler-immutable", bytes: 270 },
      { name: "compiler-core", bytes: 11_979 },
      { name: "compiler-workspace", bytes: 1_550 },
  ...
  it("executes typed scalar expressions and traps atomically as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(810_810);
    expect(outcome.cycles).toBe(7_671_788);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_395 },
      { name: "source-adapter", bytes: 99 },
      { name: "tokenizer", bytes: 820 },
      ...
      { name: "compiler-code", bytes: 10_609 },
      { name: "compiler-immutable", bytes: 270 },
      { name: "compiler-core", bytes: 10_879 },
      { name: "compiler-workspace", bytes: 1_550 },
  ...
  it("executes typed structured control as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(315_972);
    expect(outcome.cycles).toBe(3_075_409);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_395 },
      { name: "parser", bytes: 7_290 },
  ...
  it("emits packed aggregate layouts and publishes one atomic static image", async () => {
    ...
    expect(outcome.memory[outcome.symbols.ProofStatus ?? -1]).toBe(0xa5);
    expect(outcome.memory[outcome.symbols.ProofCase ?? -1]).toBe(0);
    expect(outcome.instructions).toBe(375_197);
    expect(outcome.cycles).toBe(3_515_474);
    expect(
      outcome.extents.find(({ name }) => name === "compiler-core")?.bytes,
    ).toBeLessThanOrEqual(16_384);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 7_076 },
      { name: "parser", bytes: 5_974 },
      { name: "typed-z80-sink", bytes: 1_861 },
      { name: "compiler-code", bytes: 9_336 },
      { name: "compiler-immutable", bytes: 254 },
      { name: "compiler-core", bytes: 9_590 },
      { name: "compiler-workspace", bytes: 1_550 },
