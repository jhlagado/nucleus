...
describe("manifest-driven AZM and Debug80 proofs", () => {
  it("publishes a flat target through the append-only logical sink", async () => {
    ...
    expect(outcome.instructions).toBe(885_594);
    expect(outcome.cycles).toBe(9_547_723);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 15_109 },
      { name: "compiler-immutable", bytes: 417 },
  ...
  it("fits and executes the complete host-instrumented compiler layout", async () => {
    ...
    expect(outcome.memory[outcome.symbols.ProofStatus ?? -1]).toBe(0xa5);
    expect(outcome.memory[outcome.symbols.ProofCase ?? -1]).toBe(0);
    expect(outcome.instructions).toBe(889_385);
    expect(outcome.cycles).toBe(9_589_514);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 15_165 },
      { name: "compiler-immutable", bytes: 417 },
      { name: "compiler-core", bytes: 15_582 },
      { name: "instrumented-core-gap", bytes: 8_994 },
      { name: "compiler-workspace", bytes: 3_613 },
      { name: "selected-proof-runtime", bytes: 899 },
  ...
  it("retains the historical direct-Z80 Chapter 21 module proof", async () => {
    ...
    expect(outcome.instructions).toBe(1_424_200);
    expect(outcome.cycles).toBe(14_069_055);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 14_651 },
      { name: "compiler-immutable", bytes: 417 },
      { name: "compiler-core", bytes: 15_068 },
  ...
  it("retains the historical direct-Z80 Stage 8 module proof", async () => {
    ...
    expect(outcome.instructions).toBe(1_730_834);
    expect(outcome.cycles).toBe(16_812_478);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 14_651 },
      { name: "compiler-immutable", bytes: 417 },
      { name: "compiler-core", bytes: 15_068 },
  ...
  it("traces comparison and slash tokens through multipart line endings", async () => {
    ...
    expect(outcome.cycles).toBe(35_867);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 811 },
      { name: "compiler-immutable", bytes: 417 },
      { name: "compiler-core", bytes: 1_228 },
  ...
  it("checks the scalar-local and counted-loop source slice", async () => {
    ...
    expect(outcome.instructions).toBe(37_151);
    expect(outcome.cycles).toBe(403_898);
    expect(outcome.extents).toEqual([
      { name: "compiler-code", bytes: 8_484 },
      { name: "compiler-immutable", bytes: 267 },
      { name: "compiler-core", bytes: 8_751 },
  ...
  it("executes the counted loop as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(34_384);
    expect(outcome.cycles).toBe(370_726);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_484 },
      { name: "z80-output-sink", bytes: 1_509 },
      { name: "compiler-code", bytes: 9_993 },
      { name: "compiler-immutable", bytes: 267 },
      { name: "compiler-core", bytes: 10_260 },
      { name: "compiler-workspace", bytes: 1_566 },
  ...
  it("executes checked initialized-array selection as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(50_888);
    expect(outcome.cycles).toBe(537_622);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_484 },
      { name: "source-adapter", bytes: 86 },
      { name: "tokenizer", bytes: 649 },
      { name: "semantic-sink", bytes: 88 },
      { name: "parser", bytes: 7_551 },
      { name: "z80-output-sink", bytes: 1_509 },
      { name: "compiler-code", bytes: 9_993 },
      { name: "compiler-immutable", bytes: 267 },
      { name: "compiler-core", bytes: 10_260 },
  ...
  it("executes a forward-declared recursive scalar value call", async () => {
    ...
    expect(outcome.instructions).toBe(57_020);
    expect(outcome.cycles).toBe(606_059);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_484 },
      { name: "source-adapter", bytes: 86 },
      { name: "tokenizer", bytes: 649 },
  ...
  it("executes general scalar symbols and precedence as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(108_022);
    expect(outcome.cycles).toBe(1_146_997);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_484 },
      { name: "source-adapter", bytes: 86 },
      { name: "tokenizer", bytes: 649 },
      ...
      { name: "parser", bytes: 7_551 },
      { name: "z80-output-sink", bytes: 3_352 },
      { name: "z80-expression-backend", bytes: 351 },
      { name: "compiler-code", bytes: 11_836 },
      { name: "compiler-immutable", bytes: 267 },
      { name: "compiler-core", bytes: 12_103 },
      { name: "compiler-workspace", bytes: 1_566 },
  ...
  it("executes typed scalar expressions and traps atomically as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(618_668);
    expect(outcome.cycles).toBe(6_390_223);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_484 },
      { name: "source-adapter", bytes: 86 },
      { name: "tokenizer", bytes: 649 },
      ...
      { name: "symbol-table", bytes: 110 },
      { name: "parser", bytes: 7_551 },
      { name: "typed-z80-sink", bytes: 1_843 },
      { name: "compiler-code", bytes: 10_736 },
      { name: "compiler-immutable", bytes: 267 },
      { name: "compiler-core", bytes: 11_003 },
      { name: "compiler-workspace", bytes: 1_566 },
  ...
  it("executes typed structured control as direct Z80", async () => {
    ...
    expect(outcome.instructions).toBe(249_747);
    expect(outcome.cycles).toBe(2_656_552);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 8_484 },
      { name: "parser", bytes: 7_551 },
      { name: "structured-z80-sink", bytes: 1_843 },
      { name: "compiler-code", bytes: 10_736 },
      { name: "compiler-immutable", bytes: 267 },
      { name: "compiler-core", bytes: 11_003 },
      { name: "compiler-workspace", bytes: 1_566 },
  ...
  it("emits packed aggregate layouts and publishes one atomic static image", async () => {
    ...
    expect(outcome.memory[outcome.symbols.ProofStatus ?? -1]).toBe(0xa5);
    expect(outcome.memory[outcome.symbols.ProofCase ?? -1]).toBe(0);
    expect(outcome.instructions).toBe(297_775);
    expect(outcome.cycles).toBe(3_024_941);
    expect(
      outcome.extents.find(({ name }) => name === "compiler-core")?.bytes,
    ).toBeLessThanOrEqual(16_384);
    expect(outcome.extents).toEqual([
      { name: "common-front-end", bytes: 7_165 },
      { name: "parser", bytes: 6_235 },
      { name: "typed-z80-sink", bytes: 1_889 },
      { name: "compiler-code", bytes: 9_463 },
      { name: "compiler-immutable", bytes: 251 },
      { name: "compiler-core", bytes: 9_714 },
      { name: "compiler-workspace", bytes: 1_566 },
