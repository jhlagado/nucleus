...
import { assembleLegacyAzmBinary } from "../scripts/legacy-azm-comparison.mjs";
...
const dryRunScript = path.join(
  packageRoot,
  "scripts",
  "atom-migration-dry-run.mjs",
);
const materializeScript = path.join(
  packageRoot,
  "scripts",
  "atom-migration-materialize.mjs",
);
const proofCompareScript = path.join(
  packageRoot,
  "scripts",
...
async function withPermanentAtomTranslation(context, run) {
  ...
  const translatedRoot = await mkdtemp(
    path.join(tmpdir(), "nucleus-atom-permanent-"),
  );
...
describe("Nucleus Atom migration dry-run", () => {
  it("translates Nucleus AZM directive lines to Atom forms", () => {
    ...
    expect(translateNucleusAzmLine("            .else")).toBe(
      "            %ELSE",
    );
    expect(translateNucleusAzmLine("            .endif")).toBe(
      "            %ENDIF",
    );
    expect(translateNucleusAzmLine("            .end")).toBe(
      "            ;@AZM-END",
    );
    ...
    expect(translateNucleusAzmLine("Label:.db $10")).toBe("Label:DB $10");
    ...
    expect(
      translateNucleusAzmLine(
        "LongSourceLabel: JP LongSourceLabel ; LongSourceLabel",
        {
          symbolMap,
        },
      ),
    ...
    expect(
      translateNucleusAzmLine('            .DB "LongSourceLabel"', {
        symbolMap,
      }),
    ).toBe('            DB "LongSourceLabel"');
    expect(translateNucleusAzmLine('            CP "A"')).toBe(
      "            CP 'A'",
    );
    expect(translateNucleusAzmLine('            CP "z"+1')).toBe(
      "            CP 'z'+1",
    );
    expect(translateNucleusAzmLine('            CP "\\\\"')).toBe(
      "            CP '\\\\'",
    );
    expect(
      translateNucleusAzmLine(
    ...
    ).toBe("            LD   BC,TokenRightParen<<8|TokenRightBracket");
    expect(
      translateNucleusAzmLine("AddressSpaceLimit   .equ $10000", {
        symbolMap: new Map([["AddressSpaceLimit", "N0000001"]]),
      }),
    ).toBe("N0000001   EQU 0 ;@ATOM-PROOF-LIMIT AddressSpaceLimit 65536");
    expect(
      translateNucleusAzmLine("FeatureFlag .equ 1", {
  ...
  it("keeps the checked-in Nucleus assembly tree permanent-ready without compatibility lowering", () => {
    ...
    expect(
      report.proofMatrix.filter(
        ({ status }) =>
          status !== "atom-permanent-ready" &&
          status !== "measurement-artifact",
      ),
    ).toEqual([]);
    expect(report.measured.issues).not.toHaveProperty("atom-symbol-expression");
    expect(report.measured.issues).not.toHaveProperty(
  ...
  it("assigns deterministic Atom-safe names to long symbols", async () => {
    await withTree(
      ...
      async (root) => {
        ...
        expect(
          report.ledger.map(
            ({ original, atom, permanentAtom, publicObligation }) => ({
              original,
              atom,
              permanentAtom,
              publicObligation,
        ...
        expect(
          report.ledger.find(({ original }) => original === "AnotherLongLabel"),
        ).toMatchObject({
          migrationKind: "local-label",
          permanentAtom: ".L00000",
          localScope: expect.objectContaining({
            anchor: "VeryLongPublicLabel",
          }),
        });
  ...
  it("maps conditional routine contract variants to the same following label", async () => {
    await withTree(
      ...
      async (root) => {
        ...
        expect(
          report.contractMap.map(({ line, contract, target }) => ({
            line,
            contract,
          ...
          {
            line: 2,
            contract: "in DE out A,carry",
            target: "Stage7EmitStringCheck",
          },
          {
            line: 4,
            contract: "in HL out A,carry",
            target: "Stage7EmitStringCheck",
          },
        ]);
      },
  ...
  it("does not make a local label cross a repeated global definition boundary", async () => {
    await withTree(
      ...
      async (root) => {
        ...
        expect(
          report.ledger.find(({ original }) => original === "SharedLocalReady"),
        ).toMatchObject({
      ...
      async (root) => {
        ...
        expect(report.issues).toEqual([
          expect.objectContaining({
            ...
            message:
              "numeric literal $10000 exceeds Atom's 16-bit expression range",
          }),
        ]);
      ...
      async (root) => {
        ...
        expect(report.issues).toEqual([
          expect.objectContaining({
            ...
            message:
              "symbol expression Later-Start requires preview lowering; permanent Atom source needs both symbols defined before the emitted statement",
            line: 8,
          }),
        ]);
      },
    ...
    await withTree(
      {
        ...
        "proofs/contract.json": JSON.stringify({
          source: "../asm/contract.asm",
        }),
        ...
        "proofs/dispatcher-measurement.json": JSON.stringify({
          source: "../asm/measurement.asm",
        }),
      },
      async (root) => {
        const report = scanAssembly({
        ...
        const statuses = Object.fromEntries(
          report.proofMatrix.map((entry) => [entry.proof, entry.status]),
        );
  ...
  it("classifies include-after-header targets by emitted content", async () => {
    await withTree(
      ...
      async (root) => {
        ...
        expect(
          report.includeAfterHeaderReport.byTarget
            .map(({ resolved, target }) => ({
              resolved,
              kind: target.kind,
            }))
  ...
  it("classifies include-after-header targets by nested emitted content", async () => {
    await withTree(
      {
        "asm/main.asm": ["START:", '.include "wrapper.asmi"', ""].join("\n"),
      ...
      async (root) => {
        ...
        expect(report.issues).toEqual([
          expect.objectContaining({
            ...
            message:
              "symbols collide in Atom's case-insensitive table: Name, name",
          }),
        ]);
      },
    );
  ...
  it("writes ledger, issue, and include report files from the CLI", async () => {
    await withTree(
      ...
      async (root) => {
        ...
        const migrationBundlePath = path.join(
          root,
          "out",
        ...
        const includeReport = JSON.parse(
          await readFile(includeReportPath, "utf8"),
        );
        const proofSymbolMap = JSON.parse(
          await readFile(proofSymbolMapPath, "utf8"),
        );
        const proofLimitMap = JSON.parse(
          await readFile(proofLimitMapPath, "utf8"),
        );
        ...
        const migrationBundle = JSON.parse(
          await readFile(migrationBundlePath, "utf8"),
        );
  ...
  it("writes translated Atom-preview source files from the CLI", async () => {
    await withTree(
      ...
      async (root) => {
        ...
        await expect(
          readFile(path.join(translatedRoot, "main.asm"), "utf8"),
        ).resolves.toBe(
          [
            ";@ROUTINE OUT A,CARRY CLOBBERS ZERO",
  ...
  it("selects preview or permanent Atom symbol names explicitly", () => {
    ...
    expect(
      symbolMapFromLedger(ledger, { symbols: "preview" }).get(
        "LongPublicLabel",
      ),
    ).toBe("N0000000");
    expect(
      symbolMapFromLedger(ledger, { symbols: "permanent" }).get(
        "LongPublicLabel",
      ),
    ).toBe("LNGPBLCL");
    expect(() => symbolMapFromLedger(ledger, { symbols: "unknown" })).toThrow(
      /preview or permanent/,
    );
  });
    ...
    await withTree(
      ...
      async (root) => {
        ...
        const initialCheck = spawnSync(
          process.execPath,
          [materializeScript, ...commonArgs, "--check"],
          { encoding: "utf8" },
        );
        ...
        const write = spawnSync(
          process.execPath,
          [materializeScript, ...commonArgs, "--write"],
          { encoding: "utf8" },
        );
  ...
  it("assembles the memory-map proof from permanent Atom source byte-identically", async (context) => {
    await withPermanentAtomTranslation(context, async ({ translatedRoot }) => {
      const { diagnostics, bytes: currentBin } = await assembleLegacyAzmBinary(
        path.join(asmRoot, "vertical-slice", "memory-map-proof.asm"),
      );
      ...
      expect(
        Buffer.compare(Buffer.from(atomBin), Buffer.from(currentBin)),
      ).toBe(0);
    });
  });
  ...
  it("compares one permanent Atom proof image byte-identically from the default CLI mode", () => {
    const result = spawnSync(
      process.execPath,
      [proofCompareScript, "--entry", "memory-map-proof.json"],
      {
        cwd: packageRoot,
        encoding: "utf8",
      },
  ...
  it("runs every permanent proof through the proof harness using permanent Atom source", async (context) => {
    await withPermanentAtomTranslation(
      context,
      async ({ report, translatedRoot, runPermanentAtomProof }) => {
        const readyProofs = report.proofMatrix.filter(
          ({ status }) => status === "atom-permanent-ready",
        );
        ...
        for (const row of readyProofs) {
          if (!representativeProofsByEntry.has(row.entry))
            representativeProofsByEntry.set(row.entry, row);
        }
        ...
        const structuredControlRoot = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "structured-control-z80-slice-proof.asm",
          ),
          "utf8",
        );
        expect(structuredControlRoot).toContain(
        ...
        const structuredControlProofBody = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "structured-control-z80-slice-proof-body.asmi",
          ),
          "utf8",
        );
        expect(structuredControlProofBody).toContain("SCAREC EQU");
        expect(structuredControlProofBody).toContain(
          ";@NUC-GLOBAL ProofStart PERMANENT",
        );
        ...
        expect(memoryMap.memory[memoryMap.symbols.ProofStatus ?? -1]).toBe(
          0xa5,
        );
        expect(memoryMap.symbols.AddressSpaceLimit).toBe(0x10000);
        expect(
          memoryMap.regions.reduce((total, region) => total + region.bytes, 0),
        ).toBe(65_536);
      ...
      async ({ report, translatedRoot, runPermanentAtomProof }) => {
        ...
        const helper = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "compiler-slice-code-begin.asmi",
          ),
          "utf8",
        );
  ...
  it("runs the loop compiler-slice proof from permanent Atom layout source", async (context) => {
    await withPermanentAtomTranslation(
      ...
      async ({ report, translatedRoot, runPermanentAtomProof }) => {
        ...
        expect(
          root.indexOf('%INCLUDE "loop-compiler-slice-source.asmi"'),
        ).toBeLessThan(
          root.indexOf('%INCLUDE "loop-compiler-slice-proof-body.asmi"'),
        );
        expect(
          root.indexOf('%INCLUDE "loop-compiler-slice-proof-body.asmi"'),
        ).toBeLessThan(root.indexOf('%INCLUDE "loop-compiler-slice-end.asmi"'));
        const proofBody = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "loop-compiler-slice-proof-body.asmi",
          ),
          "utf8",
        );
  ...
  it("runs the loop z80-slice proof from permanent Atom layout source", async (context) => {
    await withPermanentAtomTranslation(
      ...
      async ({ report, translatedRoot, runPermanentAtomProof }) => {
        ...
        expect(
          root.indexOf('%INCLUDE "loop-z80-slice-source.asmi"'),
        ).toBeLessThan(
          root.indexOf('%INCLUDE "loop-z80-slice-runtime-begin.asmi"'),
        );
        expect(
          root.indexOf('%INCLUDE "loop-z80-slice-runtime-after.asmi"'),
        ).toBeLessThan(
          root.indexOf('%INCLUDE "loop-z80-slice-proof-body.asmi"'),
        );
        expect(
          root.indexOf('%INCLUDE "loop-z80-slice-proof-body.asmi"'),
        ).toBeLessThan(root.indexOf('%INCLUDE "loop-z80-slice-end.asmi"'));
      ...
      async ({ report, translatedRoot, runPermanentAtomProof }) => {
        ...
        const proofBody = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "call-z80-slice-proof-body.asmi",
          ),
          "utf8",
        );
        ...
        expect(proofBody).not.toContain(
          "BadCompletionName-BadCompletionSource",
        );
      ...
      async ({ report, translatedRoot, runPermanentAtomProof }) => {
        ...
        const root = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "stage7-ll1-parser-coverage-proof.asm",
          ),
          "utf8",
        );
        ...
        const body = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "stage7-parser-coverage-proof.asmi",
          ),
          "utf8",
        );
        ...
        expect(aggregateZ80).not.toContain(
          "SymbolAggregateFlag+SymbolClassMask",
        );
        expect(aggregateZ80).not.toContain(
          "SymbolAggregateFlag+SymbolClassConstant",
        );
      ...
      async ({ report, translatedRoot, runPermanentAtomProof }) => {
        ...
        const proofBody = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "array-z80-slice-proof-body.asmi",
          ),
          "utf8",
        );
      ...
      async ({ report, translatedRoot, runPermanentAtomProof }) => {
        ...
        const root = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "expression-z80-slice-proof.asm",
          ),
          "utf8",
        );
        expect(root).toContain(
        ...
        const proofBody = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "expression-z80-slice-proof-body.asmi",
          ),
          "utf8",
        );
      ...
      async ({ report, translatedRoot, runPermanentAtomProof }) => {
        ...
        const proofBody = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "flat-target-z80-slice-proof-body.asmi",
          ),
          "utf8",
        );
        ...
        expect(proofBody).not.toContain(
          "AdapterCapturedBegin+TargetDescriptorImageBase",
        );
      ...
      async ({ report, translatedRoot, runPermanentAtomProof }) => {
        ...
        const proofBody = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "aggregate-z80-slice-proof-body.asmi",
          ),
          "utf8",
        );
        ...
        expect(proofBody).not.toContain(
          "AggregateExpectedImageEnd-AggregateExpectedImage",
        );
        expect(proofBody).not.toContain(
          "AggregateRecordStepPoint-AggregateRecordStepSource",
        );
        expect(proofBody).not.toContain(
          "AggregateDataCapacityPoint-AggregateDataCapacitySource",
        );
  ...
  it("runs the typed-expression z80-slice proof from permanent Atom layout source", async (context) => {
    await withPermanentAtomTranslation(
      ...
      async ({ report, translatedRoot, runPermanentAtomProof }) => {
        ...
        expect(
          root.indexOf('%INCLUDE "typed-expression-z80-slice-source.asmi"'),
        ).toBeLessThan(
          root.indexOf(
        ...
        const proofBody = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "typed-expression-z80-slice-proof-body.asmi",
          ),
          "utf8",
        );
        ...
        expect(proofBody).not.toContain(
          "TypedNarrowTrapPoint-TypedNarrowTrapSource",
        );
        ...
        expect(loopParserCore).not.toContain(
      ...
      async ({ report, translatedRoot, runPermanentAtomProof }) => {
        ...
        const beforeActions = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "stage7-ll1-engine-proof-before-actions.asmi",
          ),
          "utf8",
        );
        expect(beforeActions).toMatch(/\bHLL1STKL\s+EQU\s+[A-Z0-9.]+\b/);
        expect(beforeActions).not.toContain(
          "HybridLL1StackBase+HybridLL1StackCapacity",
        );
    ...
    await withPermanentAtomTranslation(
      ...
      async ({ report, translatedRoot, runPermanentAtomProof }) => {
        const dependent = report.proofMatrix.find(
          ({ proof }) =>
            proof === "stage7-ll1-aggregate-call-z80-slice-proof.json",
        );
        const aggregateCallBlockers = dependent?.blockers.filter(
          ({ file }) =>
        ...
        const stage7Root = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "aggregate-call-parser-stage7.asm",
          ),
          "utf8",
        );
        expect(stage7Root).toContain(
        ...
        const core = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "aggregate-call-parser-core.asmi",
          ),
          "utf8",
        );
        ...
        expect(actions).not.toContain(
          "SymbolRecordTypeFlag+SymbolAggregateFlag",
        );
        ...
        expect(aggregateParser).not.toContain(
          "SymbolRecordTypeFlag+SymbolAggregateFlag",
        );
        ...
        const expressionBlockers = dependent?.blockers.filter(
          ({ code, file }) =>
            (code === "atom-symbol-expression" ||
              code === "include-after-header") &&
        ...
        const typedExpressionParser = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "typed-expression-parser.asm",
          ),
          "utf8",
        );
        expect(typedExpressionParser).toContain(
        ...
        expect(typedExpressionParserCore).not.toContain(
          "SemanticOr8*$100+SemanticOr16",
        );
        expect(typedExpressionParserCore).not.toContain(
          "ScalarMetaConstant+ScalarTypeU8",
        );
        ...
        expect(typedExpressionZ80).toContain(
        ...
        const typedExpressionZ80Core = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "typed-expression-z80-core.asmi",
          ),
          "utf8",
        );
        expect(typedExpressionZ80Core).toContain("TEZACTD EQU");
        expect(typedExpressionZ80Core).not.toContain(
          '%INCLUDE "structured-control-z80.asm"',
        );
        expect(typedExpressionZ80Core).not.toContain(
          "ActivationDepth-StateBase",
        );
        ...
        expect(loopKeywords).not.toContain(
          "Stage8CallableServiceFlag+Stage8ServiceResultU8",
        );
      },
    );
  });
      ...
      async ({ report, translatedRoot }) => {
        ...
        const root = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "stage7-ll1-aggregate-call-z80-slice-proof.asm",
          ),
          "utf8",
        );
        expect(root).toContain(
        ...
        const proofBody = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "stage7-ll1-aggregate-call-proof-body.asmi",
          ),
          "utf8",
        );
        ...
        expect(proofBody).not.toContain(
          "Stage7CorruptStringLengthPoint-Stage7CorruptStringSource",
        );
        ...
        const { diagnostics, bytes: currentBin } =
          await assembleLegacyAzmBinary(
            path.join(
              asmRoot,
              "vertical-slice",
              "stage7-ll1-aggregate-call-z80-slice-proof.asm",
      ...
      async ({ report, translatedRoot }) => {
        ...
        const root = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "stage8-failure-z80-slice-proof.asm",
          ),
          "utf8",
        );
        ...
        const proofBody = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "stage8-failure-proof-body.asmi",
          ),
          "utf8",
        );
      ...
      async ({ report, translatedRoot }) => {
        ...
        const root = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "stage9-conformance-z80-slice-proof.asm",
          ),
          "utf8",
        );
        expect(root).toContain(
        ...
        const proofBody = await readFile(
          path.join(
            translatedRoot,
            "vertical-slice",
            "stage9-conformance-proof-body.asmi",
          ),
          "utf8",
        );
    ...
    await withPermanentAtomTranslation(context, async ({ translatedRoot }) => {
      const root = await readFile(
        path.join(
          translatedRoot,
          "vertical-slice",
          "nucleus-target-runtime-link.asm",
        ),
        "utf8",
      );
      expect(root).toContain(
      ...
      expect(
        root.indexOf('%INCLUDE "nucleus-target-runtime-link-begin.asmi"'),
      ).toBeLessThan(root.indexOf('%INCLUDE "target-z80-runtime.asm"'));
    ...
    await withTree(
      {
        ...
        "asm/lib.asmi": ["LibLongLabel:", "            .db 1", ""].join("\n"),
        "proofs/main.json": "{}",
      },
      async (root) => {
        const report = scanAssembly({
    ...
    await withTree(
      {
        ...
        "grammar/generated.asmi": ["GeneratedLongLabel:", "RET", ""].join("\n"),
        "proofs/main.json": "{}",
      },
      async (root) => {
        const report = scanAssembly({
  ...
  it("splits flattened Atom-preview entries into ordered native source parts", async () => {
    await withTree(
      ...
      async (root) => {
        ...
        expect(
          parts.every(({ compilerBytes }) => compilerBytes.length <= 40),
        ).toBe(true);
        expect(
  ...
  it("derives generated preview constants from earlier EQU aliases", () => {
    const values = augmentSymbolValuesFromPreview(
      ["BASE EQU $5000", "SIZE EQU 54", "END  EQU BASE+SIZE", ""].join("\n"),
      new Map(),
    );
  ...
  it("masks unresolved preview EQU definitions without inventing aliases", () => {
    expect(
      lowerResolvedPreviewExpressions(
        ["END EQU MISSING+SIZE", "LD HL,END", ""].join("\n"),
        new Map([["SIZE", 54]]),
      ),
    ).toBe(
      [";@UNRESOLVED-EQU END EQU MISSING+SIZE", "LD HL,END", ""].join("\n"),
    );
  });
  ...
  it("accepts legacy unordered output while still rejecting image overlap", () => {
    ...
    expect(
      sink.begin({
        target: { start: 0x1000, capacity: 0x1000 },
        descriptor: 0x4000,
      }),
    ).toBe(0);
    expect(
      sink.image({ bank: 0, address: 0x1800, bytes: Uint8Array.of(1) }),
    ).toBe(0);
    expect(
      sink.image({ bank: 0, address: 0x1000, bytes: Uint8Array.of(2) }),
    ).toBe(0);
    expect(
      sink.image({ bank: 0, address: 0x1000, bytes: Uint8Array.of(3) }),
    ).toBe(0xe2);
