...
The external packaging layer owns physical files, filenames, dependency discovery, dependency ordering, duplicate suppression, and source transport. It must resolve or reject missing physical inputs and must not present duplicate stable source identities. These are packaging failures, not Nucleus source diagnostics, and the core compiler need not diagnose a host filesystem failure. Nucleus 0.1 has no `import`, `include`, `module`, or namespace token or declaration. Section 4.3.2 defines a standard packaging directive written inside an ordinary `//` comment; it does not enter the grammar.
...
#### 4.3.2 Import-directed packaging

The standard dependency-discovery convention recognizes a leading comment of the form `//% import "relative/path.nu"`. Blank lines, ordinary line comments, and import comments may form a file header. The first line containing Nucleus source ends that header. A packaging layer rejects a malformed directive, a directive after the header, a missing input, a dependency cycle, a project-root escape, or conflicting physical identities before invoking the compiler.

The directive remains part of the source bytes. The tokenizer processes it as the ordinary comment defined by Section 3.4 and therefore emits no import token. The packaging layer must not strip it, normalize its line endings, concatenate its dependency anonymously, or otherwise rewrite the source part. Its normalized project-relative path supplies the stable diagnostic and debugging identity.

Resolution begins at one external entry file. It visits imports in written order, emits every dependency before its importer, and emits one source part for each physical file. Duplicate imports and diamond dependencies therefore contribute one part. The resulting order remains ordinary declaration order under Section 4.5; the compiler neither constructs nor retains the dependency graph.

The separate source-packaging contract defines the generated SP1 source-plan representation used by filesystem-aware hosts. That representation and project JSON remain outside Nucleus source semantics.
...
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Source       | Ordered multipart compilation input with stable part identities and part-relative diagnostics; explicit flat manifest or host-resolved `//% import` comment headers; ASCII-compatible bytes, `//` comments, logical newlines, case-sensitive preserved names, lowercase keywords, decimal, hexadecimal, and binary integers, byte characters, bounded string literals, fixed punctuation. |
