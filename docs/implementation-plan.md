...
The operating layer owns the canonical runtime helper bytes. The compiler
retains only the runtime identity, expected length, vector layout, and helper
offsets. At each derived runtime address it calls a bounded runtime-image
provider, which appends the selected bytes to the image spool as ordinary
`IMAGE` records and reports the exact length. The provider blob and provider
implementation are external-service resources, not compiler core or workspace;
the compiler-side call path must still be measured, and every emitted per-bank
copy must be reported as selected-runtime bytes and as occupancy in its bank
image.
...
1. add the compact target descriptor, runtime-provider lookup, and
   identity/length rejection without changing generated bytes;
