...
| decoded string-literal bytes             |        255 | one-byte token length; source spelling remains retained                               | lexical diagnostic before wrap or truncation                                     | accepted 255-byte literal and rejected 256th decoded byte                         |
| lexical delimiter nesting                |        255 | one-byte depth; the replacement also retains one full kind byte per open delimiter   | lexical diagnostic on first excess, unmatched, mismatched, or unclosed delimiter | accepted depth 255, rejected depth 256, and crossed-kind discriminators           |
