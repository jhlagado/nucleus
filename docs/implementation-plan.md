...
The sealed-string pass caps `string[N]` at 254, gives every string a permanent
zero byte after its payload capacity, and extends the selected aggregate and
static-image limits to 256 bytes. A retained zero extent denotes 256 inside
compiler metadata; source string lengths still range only from zero through
the declared capacity. Word static-image cursors add four workspace bytes, and
runtime length checks reject a corrupted `L = 255` before `.length` or indexing
can use it. The representative Stage 7 string program grows from 368 to 557
generated bytes: one static-data byte and 188 bytes of emitted complete-region,
length-invariant, and index validation. The largest Chapter 21 program grows
from 945 to 1,019 bytes for the same reason.

Fresh assembly measures 13,616 code bytes plus 368 immutable bytes, for a
...
| program static-image bytes              |        256 | private compiler image plus a retained word length                            | capacity diagnostic                                                              | complete 256-byte string object and rejected following allocation            |
