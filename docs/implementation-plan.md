...
- all owned aggregate storage belongs to top-level program objects and their
  inline subobjects;
- aggregate parameters use typed, opaque address carriers, while routine results are scalar or absent;
...
### Stage 7: aggregate calls and copying

Add aggregate parameter carriers, checked selection, and exact-type aggregate
assignment. Aggregate-producing routines write into caller-provided storage
through a fixed aggregate parameter; routine results are scalar or absent.
...
Field and array selection derive checked aggregate carriers, while bounded-
string `length` produces a read-only scalar value.
...
- no source operation can inspect, store, convert, compare, or calculate with
  an address carrier;
- aggregate-producing routines copy into caller-supplied destinations;
- nested calls preserve live aggregate arguments;
...
### Scalar-only routine results

Routine results are now absent or scalar. Aggregate-producing routines receive
caller-provided destination storage through the existing fixed aggregate-
parameter mechanism. The compiler retains aggregate arguments, checked paths,
exact-type copying, 16-bit extents, scalar results, result-free calls, and
recoverable failure, while removing the transient returned-alias category.

The correctness form measured 14,239 compiler-code bytes plus 390 immutable
bytes, for a 14,629-byte compiler core: 72 bytes below the 14,701-byte baseline.
Workspace remains 3,623 bytes and the selected runtime remains 655 bytes. The
final 9,128-byte parser consists of a 230-byte LL(1) engine, 759 generated table
bytes, 2,737 action bytes, and 5,402 bytes of residual expression, name, and
type islands. The largest Chapter 21 program is 1,062 generated bytes. The
expanded Stage 7 proof occupies 3,162 bytes and executes 1,827,919 instructions
in 16,909,133 T-states; the Stage 8 and Stage 9 proofs execute 1,582,191 /
14,759,878 and 1,530,213 / 14,418,613 instructions / T-states respectively.

The focused post-correctness pass saves 30 more compiler bytes. Aggregate-value
parsing now routes through the retained root emitter once; the resulting
single-caller symbol lookup is inlined. The scalar-return path drops its retired
aggregate-result branch, inlines its one remaining failure-consumption helper,
and removes one redundant flag reset. Removing that branch also leaves the
scalar-path finisher with one caller, where it is inlined. The final compiler
account is therefore 14,209 code bytes plus 390 immutable bytes, or 14,599 core
bytes: 102 bytes below the preceding 14,701-byte baseline.

The machine-readable grammar retains 40 nonterminals, 75 productions, 72
action or external entries, and 239 right-side symbols. Its prediction cells
fall from 145 to 144, generated tables from 760 to 759 bytes, and the conflict
set remains empty. Two semantic operations and two dispatch entries disappear.
No call-frame byte, workspace cell, grammar action-directory entry, or separate
generated return template existed solely for aggregate results, so those
