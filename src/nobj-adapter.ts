...
const prepareNobjAdapterGeneration = ({
  name,
  producerMemory,
  start,
  length,
  ...
  onImageByte,
}: NobjAdapterGeneration): NobjGenerationSink => {
  if (length > maxBytes) {
    throw new NobjError(
...
export const commitNobjAdapterGeneration = (
  generation: NobjAdapterGeneration,
): Uint8Array => prepareNobjAdapterGeneration(generation).commit();

export const commitNobjAdapterGenerationTo = async (
  generation: NobjAdapterGeneration,
  output: NobjSequentialOutput,
): Promise<NobjCommitMetadata> =>
  prepareNobjAdapterGeneration(generation).commitTo(output);
