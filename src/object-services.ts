...
export type NucleusSystemStatusCode =
  (typeof NucleusSystemStatus)[keyof typeof NucleusSystemStatus];
...
type ObjectHandle = ReadHandle | WriteHandle;
...
const readDword = (memory: Uint8Array, at: number): number =>
...
const rangeIsValid = (
  ...
  pointer: number,
  length: number,
): boolean => pointer >= 0 && length >= 0 && pointer + length <= memory.length;
...
const statusForError = (error: unknown): NucleusSystemStatusCode => {
...
const requestHasZero = (
  memory: Uint8Array,
  request: number,
  offsets: readonly number[],
): boolean => offsets.every((offset) => memory[request + offset] === 0);
...
export class NodeNamedObjectServices {
  ...
  readonly #maxHandles: number;
  readonly #handles = new Map<number, ObjectHandle>();
  #nextHandle = 1;
  #temporaryOrdinal = 0;

  public constructor(
    root: string,
    options: NodeNamedObjectServicesOptions = {},
  ) {
  ...
  public dispatch(memory: Uint8Array, request: number): NucleusSystemStatusCode {
    ...
    if (
      memory[request + NucleusObjectRequest.size] !==
    ...
    const handleNumber = readWord(
      memory,
      request + NucleusObjectRequest.handle,
    );
      ...
      switch (operation) {
        ...
        case NucleusObjectOperation.beginWrite:
          ...
          return this.#open(
            ...
            pointer,
            length,
            operation === NucleusObjectOperation.beginWrite,
          );
        ...
        case NucleusObjectOperation.write:
          ...
          return this.#transfer(
            ...
            pointer,
            length,
            operation === NucleusObjectOperation.write,
          );
  ...
  #allocate(state: ObjectHandle): number | undefined {
    if (this.#handles.size >= this.#maxHandles) return undefined;
  ...
  #name(memory: Uint8Array, pointer: number, length: number): string | undefined {
    if (!rangeIsValid(memory, pointer, length)) return undefined;
  ...
  #path(name: string, writing: boolean): string {
  ...
  #open(
    ...
    if (name === undefined) return NucleusSystemStatus.invalid;
    const target = this.#path(name, writing);
    if (
      writing &&
      [...this.#handles.values()].some(
        (state) => state.kind === "write" && state.target === target,
      )
    ) {
    ...
    if (handle === undefined) {
      if (state.kind === "read") closeSync(state.descriptor);
      else {
        closeSync(state.descriptor!);
  ...
  #transfer(
    ...
    if (writing && state.kind !== "write") return NucleusSystemStatus.invalid;
    if (length === 0) return NucleusSystemStatus.success;
    const bytes = memory.subarray(pointer, pointer + length);
    if (!writing) {
    ...
    if (state.kind !== "write") return NucleusSystemStatus.invalid;
    ...
    try {
      ...
      while (written < length) {
        ...
        if (count === 0) throw new Error("zero-byte object write");
        written += count;
      }
  ...
  #seek(handleNumber: number, offset: number): NucleusSystemStatusCode {
    const state = this.#handles.get(handleNumber);
  ...
  #terminal(operation: number, handleNumber: number): NucleusSystemStatusCode {
    const state = this.#handles.get(handleNumber);
    if (state === undefined) return NucleusSystemStatus.invalid;
    ...
    if (operation === NucleusObjectOperation.close) {
      if (state.kind !== "read") return NucleusSystemStatus.invalid;
      closeSync(state.descriptor);
  ...
  #abort(handleNumber: number, state: WriteHandle): NucleusSystemStatusCode {
    this.#handles.delete(handleNumber);
    let status: NucleusSystemStatusCode = NucleusSystemStatus.success;
    if (state.descriptor !== undefined) {
      try {
        closeSync(state.descriptor);
      } catch (error) {
    ...
    try {
      if (existsSync(state.temporary)) unlinkSync(state.temporary);
    } catch (error) {
      status = statusForError(error);
    }
    return status;
