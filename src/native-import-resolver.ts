...
import {
  nativeImportResolverHex,
  nativeImportResolverSymbols,
} from "./generated-native-import-resolver.js";
import {
 ...
 */
export const runNativeImportResolver = (
  services: NodeNamedObjectServices,
  entry: string,
): NativeImportResolverResult => {
  ...
  runtime = createZ80Runtime(
    { memory, startAddress: symbol("NativeImportResolve") },
    symbol("NativeImportResolve"),
    {
      write: (port) => {
        if ((port & 0xff) !== gatewayPort) {
          throw new Error(
            `native import resolver wrote unexpected port ${port & 0xff}`,
          );
        }
        if (runtime.cpu.c !== symbol("NucleusServiceObject")) {
          throw new Error(
            `native import resolver requested unexpected service ${runtime.cpu.c}`,
          );
        }
  ...
  const status = runtime.cpu.a as NucleusSystemStatusCode;
  const success =
    runtime.cpu.flags.C === 0 && status === NucleusSystemStatus.success;
  if (!success) services.abortAll();
  return {
    success,
    status,
    instructions,
