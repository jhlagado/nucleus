...
const executableContextKey = (
  identity: number,
  runtimeBase: number,
  writableStateBase: number,
  packetService: number,
): string =>
  JSON.stringify([identity, runtimeBase, writableStateBase, packetService]);
...
const catalogImage = (
  entry: (typeof generatedRuntimeCatalog)[number],
): RuntimeImage => {
  if (
    entry.vectorLength !== runtimeVectorLength ||
    ...
    throw new NobjError(
      `generated runtime catalog entry ${entry.name} is invalid`,
    );
  }
  ...
  return {
    ...
    bytes: parseIntelHex(entry.hex).memory.slice(
      entry.runtimeBase,
      entry.runtimeBase + entry.expectedLength,
    ),
...
export const bundledRuntimeProvider: RuntimeImageProvider = {
...
export const bundledRuntimeCatalog = generatedRuntimeCatalog.map(
  ({ name, runtimeBase, stateBase, packetService }) => ({
    name,
    runtimeBase,
