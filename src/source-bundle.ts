...
const validateBoundary = (bytes: Uint8Array, name: string): void => {
  ...
  if (quote !== 0) fail(`${name} has an unterminated literal at its boundary`);
  if (delimiters.length !== 0) fail(`${name} ends inside a delimiter`);
};

const validateBank = (bank: number, name: string): void => {
  if (!Number.isInteger(bank) || bank < 0 || bank > 0xff) {
    fail(`${name} has a bank outside 0..255`);
  }
};

export const prepareNucleusSourceBundle = (
  sources: readonly NucleusSourceFile[],
): NucleusSourceBundle => {
  ...
  for (const source of sources) {
    ...
    if (names.has(source.name)) fail(`duplicate source name ${source.name}`);
    ...
    if (byteLength > nucleusSourceByteCapacity) {
      fail(
        `concatenated source contains ${byteLength} bytes; capacity is ${nucleusSourceByteCapacity}`,
      );
    }
    prepared.push({
      bytes,
...
export const mapNucleusSourceBundleOffset = (
  bundle: NucleusSourceBundle,
  offset: number,
): NucleusMappedSourcePosition => {
  ...
  if (file === undefined) {
    throw new NucleusSourceBundleError(
      `source offset ${offset} has no source file`,
    );
  }
