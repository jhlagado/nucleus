...
const positiveInteger = (text: string, option: string): number => {
...
const parseRunArguments = (args: string[]): RunCliArguments => {
  const parsed: RunCliArguments = {};
  while (args.length > 0) {
    const argument = args.shift();
    if (argument === "--target-profile") {
      ...
      parsed.maxInstructions = positiveInteger(
        valueAfter(args, argument),
        argument,
      );
...
const run = async (args: string[]): Promise<number> => {
  ...
  const target =
    parsed.targetProfile === undefined
      ? {}
      : parseNucleusTargetProfile(
          await readFile(parsed.targetProfile, "utf8"),
          { requireServices: true },
        );
  const fileInput =
    parsed.input === undefined ? undefined : await readFile(parsed.input);
  ...
  switch (result.outcome) {
    ...
    case "trap":
      console.error(
        `Nucleus program ${result.outcome}: reason ${result.trapReason}, error ${result.errorCode}, source offset ${result.trapOffset}`,
      );
      return 1;
    case "loaderFailure":
      console.error(
        `NOBJ load failed: outcome ${result.loaderOutcome}, status ${result.status}, record ${result.recordOrdinal}`,
      );
      return 2;
    case "executionLimit":
      console.error(
        `Nucleus ${result.phase} execution limit reached at $${result.programCounter.toString(16).padStart(4, "0")}`,
      );
      return 2;
  }
};
