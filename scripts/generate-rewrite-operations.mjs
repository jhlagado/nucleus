...
const unorderedDescriptors = operations.map((operation) => {
...
// Semantic ordinals are private compiler data. Grouping them by record width
// lets the Z80 decoder recover every width from a short cumulative-limit table
// instead of retaining one byte (or nibble) per operation. Array#sort is stable,
// so operations of the same width retain their source-authority order.
const descriptors = unorderedDescriptors
  .toSorted((left, right) => left.width - right.width)
  .map((descriptor, zeroBasedIndex) => ({
    ...descriptor,
    id: zeroBasedIndex + 1,
  }));
...
const maximumRecordWidth = Math.max(
  ...descriptors.map(({ width }) => width),
);
const cumulativeWidthLimits = Array.from(
  { length: maximumRecordWidth },
  (_, zeroBasedWidth) =>
    descriptors.filter(({ width }) => width <= zeroBasedWidth + 1).length,
);
...
const asm = [
  ...
  `RewriteSemanticMaximumRecordWidth .equ ${maximumRecordWidth}`,
  ...
  "",
  "RewriteSemanticOperationWidthLimits:",
  ...cumulativeWidthLimits.map(
    (limit, index) =>
      `            .db ${hexByte(limit)} ; widths through ${index + 1}`,
  ),
  "RewriteSemanticOperationWidthLimitsEnd:",
