...
for (const operation of operations) {
  ...
  if (kind === "escape" && !escapeSelectors.includes(selector)) {
    escapeSelectors.push(selector);
  }
}
...
for (const selector of Object.keys(recipeSource.escapes ?? {})) {
  if (!escapeSelectors.includes(selector)) {
    throw new Error(`unknown escape selector ${selector}`);
  }
  if (!/^RewriteBackendEscape[A-Z][A-Za-z0-9]*$/.test(recipeSource.escapes[selector])) {
    throw new Error(`invalid escape handler ${recipeSource.escapes[selector]}`);
  }
}

const supportedOperationCount = operations.filter((operation) => {
  const [kind, selector] = operation.backend ?? [];
  return kind === "recipe"
    ? recipeSource.selectors[selector] !== undefined
    : recipeSource.escapes?.[selector] !== undefined;
}).length;
...
const asm = [
  ...
  `RewriteBackendSupportedOperationCount .equ ${supportedOperationCount}`,
  "",
  ...
  "",
  "RewriteBackendEscapeDirectory:",
  ...escapeSelectors.map((selector) =>
    recipeSource.escapes?.[selector] === undefined
      ? `            .dw 0 ; ${selector}`
      : `            .dw ${recipeSource.escapes[selector]} ; ${selector}`,
  ),
  "RewriteBackendEscapeDirectoryEnd:",
  "",
